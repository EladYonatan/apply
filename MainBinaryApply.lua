...
require 'eladtools'
require 'trepl'
require 'adam_binary_clip_b_apply'
require 'sgd_clip_b'
...
cmd:option('-checkpoint',         1e5,                        'Save a weight check point every n samples. 0 for off')
...
local loss = modelAll.loss or nn.ClassNLLCriterion()
...
local optimizer = Optimizer{
  ...
  --  OptFunction = _G.optim[opt.optimization],
    --OptFunction = adaMax_binary_clip_shift,
    OptFunction =adam_binary_clip_b,
  --  OptFunction =sgd_clip_b,
    GradRenorm = 1,
    OptState = optimState,
...
local function Forward(DB, train)
    ...
    while NumSamples < SizeData do
        ...
        while MiniBatch:getNextBatch() do
            ...
            --one_hot_yt=torch.zeros(yt:size(1),1000)
        ...
        if train and opt.checkpoint >0 and (currBatch % math.ceil(opt.checkpoint/opt.bufferSize) == 0) then
            ...
            print('\nAfter ' .. NumSamples .. ' samples, current loss is: ' .. lossVal .. '\n')
...
while epoch ~= opt.epoch do
    ...
    print(Weights:max(),Gradients:max())
    --optimState.learningRate= optimState.learningRate*0.95
    optimState.learningRate=optimState.learningRate*0.72
    --if epoch==2 or epoch%7==0  then
